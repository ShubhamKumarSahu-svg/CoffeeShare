        ...
        try {
          ...
          switch (message.type) {
            case MessageType.RequestInfo: {
              ...
              const fileInfo = filesRef.current.map((f) => {
                return {
            ...
            case MessageType.Start: {
              ...
              validateOffset(filesRef.current, currentFileName, currentOffset)
            ...
            case MessageType.Resume: {
              console.log('[UploaderConnections] transfer resumed')
              
              updateConnection((draft) => {
                if (draft.status !== UploaderConnectionStatus.Paused) return draft
                return { ...draft, status: UploaderConnectionStatus.Uploading }
              })
